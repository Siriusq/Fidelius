(function (root, factory) {
  ...
  function loadWasmModule() {
    ...
    if (typeof require === 'function') {
      return Promise.resolve(require('../js/dist/argon2.js'));
    }
  ...
  function loadWasmBinary() {
    ...
    if (typeof require === 'function') {
      return Promise.resolve(require('../js/dist/argon2.wasm')).then(
        (wasmModule) => {
   ...
   * @param {number} [params.time=3] - the number of iterations
   * @param {number} [params.mem=65536] - used memory, in KiB
  ...
  function argon2Hash(params) {
    const mCost = params.mem || 65536;
    return loadModule(mCost).then((Module) => {
      const tCost = params.time || 3;
